...
content = content.replace( // this will replace the second occurrence of eq(members.organizationId, orgId) inside getMembershipEventsService
  'eq(members.organizationId, orgId)',
  'tenantWhere(members, ctx)'
);

// 3. listMembershipEventsService
content = content.replace(
...
content = content.replace( // there are two in listMembershipEventsService
  'eq(members.organizationId, orgId)',
  'tenantWhere(members, ctx)'
);

// 4. createMembershipService
content = content.replace(
